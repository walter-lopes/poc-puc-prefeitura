...
namespace SGM_MVC.SiteUtils
{
    public static class Formatacao
    {
        ...
        {
           return DateTime.TryParse(date, out DateTime dateTime) ? DateTime.Parse(date).ToString(pattern) : "";
        }
        public static DateTime StringDateToDate(string date)
        {
            return DateTime.TryParse(date, out DateTime dateTime) ? DateTime.Parse(date) : DateTime.Parse("1900/01/01");
        }
    }
}
